...
public class SkillTreeData {
  ...
  public SkillLearnTemplate[] getTemplatesFor(PlayerClass playerClass, int level, Race race) {
    ...
    List<SkillLearnTemplate> classRaceSpecificTemplates = (List<SkillLearnTemplate>) this.templates
        .get(makeHash(playerClass.ordinal(), race.ordinal(), level));

    List<SkillLearnTemplate> classSpecificTemplates = (List<SkillLearnTemplate>) this.templates
        .get(makeHash(playerClass.ordinal(), SkillRace.ALL.ordinal(), level));

    List<SkillLearnTemplate> generalTemplates = (List<SkillLearnTemplate>) this.templates
        .get(makeHash(SkillClass.ALL.ordinal(), SkillRace.ALL.ordinal(), level));
  ...
  public int size() {
    int size = 0;
    for (int arr$[] = this.templates.keys(), len$ = arr$.length, i$ = 0; i$ < len$;) {
      Integer key = Integer.valueOf(arr$[i$]);
      size += ((ArrayList) this.templates.get(key.intValue())).size();
      i$++;
    }
    return size;
  }
