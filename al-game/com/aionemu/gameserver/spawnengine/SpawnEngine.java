...
public class SpawnEngine {
  ...
  public VisibleObject spawnObject(SpawnTemplate spawn, int instanceIndex) {
    ...
    if (npcTemplate instanceof NpcTemplate) {
      Monster monster;
      Npc npc1;
      BindpointController bindPointController;
      ...
      switch (npcType) {
        ...
        case RIFT:
        case STATIC:
          monster = new Monster(iDFactory.nextId(), new MonsterController(), spawn,
              (VisibleObjectTemplate) npcTemplate);

          monster.setKnownlist(new KnownList((VisibleObject) monster));
          break;
        case null:
          npc1 = new Npc(iDFactory.nextId(), (NpcController) new PostboxController(), spawn,
              (VisibleObjectTemplate) npcTemplate);
        ...
        case null:
          ...
          npc1 = new Npc(iDFactory.nextId(), (NpcController) bindPointController, spawn,
              (VisibleObjectTemplate) npcTemplate);
          npc1.setKnownlist((KnownList) new StaticObjectKnownList((VisibleObject) npc1));
          break;
        case null:
          npc1 = new Npc(iDFactory.nextId(), (NpcController) new ActionitemController(), spawn,
              (VisibleObjectTemplate) npcTemplate);

          npc1.setKnownlist((KnownList) new StaticObjectKnownList((VisibleObject) npc1));
          break;
        case null:
          npc1 = new Npc(iDFactory.nextId(), (NpcController) new PortalController(), spawn,
              (VisibleObjectTemplate) npcTemplate);
    ...
    if (npcTemplate instanceof GatherableTemplate) {
      ...
      Gatherable gatherable = new Gatherable(spawn, (VisibleObjectTemplate) npcTemplate, iDFactory.nextId(),
  ...
  public Kisk spawnKisk(SpawnTemplate spawn, int instanceIndex, Player creator) {
    ...
    Kisk kisk = new Kisk(IDFactory.getInstance().nextId(), (NpcController) new KiskController(), spawn, template,
        creator);
  ...
  public Servant spawnServant(SpawnTemplate spawn, int instanceIndex, Creature creator, int skillId, int hpRatio) {
    int objectId = spawn.getSpawnGroup().getNpcid();
    NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
    Servant servant = new Servant(IDFactory.getInstance().nextId(), (NpcController) new ServantController(), spawn,
        (VisibleObjectTemplate) npcTemplate);
  ...
  public Summon spawnSummon(Player creator, int npcId, int skillLvl) {
    ...
    Summon summon = new Summon(IDFactory.getInstance().nextId(), (CreatureController) new SummonController(), spawn,
        (VisibleObjectTemplate) npcTemplate, statsTemplate, level);
  ...
  public SpawnTemplate addNewSpawn(int worldId, int instanceId, int objectId, float x, float y, float z, byte heading,
      int walkerid, int randomwalk, boolean noRespawn) {
    return addNewSpawn(worldId, instanceId, objectId, x, y, z, heading, walkerid, randomwalk, noRespawn, false);
  }

  public SpawnTemplate addNewSpawn(int worldId, int instanceId, int objectId, float x, float y, float z, byte heading,
      int walkerid, int randomwalk, boolean noRespawn, boolean isNewSpawn) {
