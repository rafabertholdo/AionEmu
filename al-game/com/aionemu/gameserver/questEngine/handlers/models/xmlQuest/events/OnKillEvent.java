  ...
  public boolean operate(QuestEnv env) {
    ...
    for (MonsterInfo monsterInfo : this.monsterInfos) {
      ...
      if (monsterInfo.getNpcId() == npc.getNpcId()) {
        ...
        if (var >= ((monsterInfo.getMinVarValue() == null) ? 0 : monsterInfo.getMinVarValue().intValue()) && var < monsterInfo.getMaxKill()) {
          
          qs.setQuestVarById(monsterInfo.getVarId(), var + 1);
          PacketSendUtility.sendPacket(env.getPlayer(), (AionServerPacket)new SM_QUEST_ACCEPTED(env.getQuestId().intValue(), qs.getStatus(), qs.getQuestVars().getQuestVars()));
        } 
      } 
    } 
