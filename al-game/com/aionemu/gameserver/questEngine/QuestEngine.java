...
public class QuestEngine
{
  ...
  public boolean onDialog(QuestEnv env) {
    ...
    if (env.getQuestId().intValue() != 0) {
      ...
      if (questHandler != null && 
        questHandler.onDialogEvent(env)) {
        return true;
      }
    } else {
      
      Npc npc = (Npc)env.getVisibleObject();
      for (Iterator<Integer> i$ = getNpcQuestData((npc == null) ? 0 : npc.getNpcId()).getOnTalkEvent().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
  ...
  public boolean onKill(QuestEnv env) {
    Npc npc = (Npc)env.getVisibleObject();
    for (Iterator<Integer> i$ = getNpcQuestData(npc.getNpcId()).getOnKillEvent().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
  ...
  public boolean onAttack(QuestEnv env) {
    ...
    for (Iterator<Integer> i$ = getNpcQuestData(npc.getNpcId()).getOnAttackEvent().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
  ...
  public boolean onItemUseEvent(QuestEnv env, Item item) {
    ...
    for (int index = 0; index < lists.size(); index++) {
      ...
      if (questHandler != null && 
        questHandler.onItemUseEvent(env, item))
        return true; 
    } 
    return false;
  }
  ...
  public void onQuestTimerEnd(QuestEnv env) {
    for (Iterator<Integer> i$ = this.questOnQuestTimerEnd.iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
