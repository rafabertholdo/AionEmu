...
public class MoveController
{
  ...
  private void move() {
    ...
    if (dist > this.distance) {
      ...
      if (this.directionChanged) {
        ...
        PacketSendUtility.broadcastPacket((VisibleObject)this.owner, (AionServerPacket)new SM_MOVE(this.owner.getObjectId(), ownerX, ownerY, ownerZ, (float)(x2 * dist) + ownerX, (float)(y2 * dist) + ownerY, (float)(z2 * dist) + ownerZ, heading, MovementType.UNKNOWN_NPC_MOVEMENT));
  ...
  public double getDistanceToTarget() {
    if (this.isFollowTarget) {
      ...
      VisibleObject target = this.owner.getTarget();
      if (target != null) {
        return MathUtil.getDistance(this.owner.getX(), this.owner.getY(), this.owner.getZ(), target.getX(), target.getY(), target.getZ());
      }
    } 
    
    return MathUtil.getDistance(this.owner.getX(), this.owner.getY(), this.owner.getZ(), this.targetX, this.targetY, this.targetZ);
  }
