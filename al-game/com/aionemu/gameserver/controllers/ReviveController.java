...
public class ReviveController {
  ...
  public void skillRevive() {
    revive(10, 10);
    PacketSendUtility.broadcastPacket(this.player,
        (AionServerPacket) new SM_EMOTION((Creature) this.player, EmotionType.RESURRECT), true);
  ...
  public void itemSelfRevive() {
    ...
    this.player.addItemCoolDown(item.getItemTemplate().getDelayId(), System.currentTimeMillis() + useDelay,
        useDelay / 1000);
    PacketSendUtility.broadcastPacket(this.player,
        (AionServerPacket) new SM_ITEM_USAGE_ANIMATION(this.player.getObjectId(), item.getObjectId(),
