...
public class NpcController extends CreatureController<Npc> {
    ...
    if (object instanceof Player) {
      ...
      if (owner.getMoveController().isWalking()) {
        PacketSendUtility.sendPacket((Player) object,
            (AionServerPacket) new SM_EMOTION((Creature) owner, EmotionType.WALK));
      }
  ...
  public void onDie(Creature lastAttacker) {
    ...
    PacketSendUtility.broadcastPacket((VisibleObject) owner, (AionServerPacket) new SM_EMOTION((Creature) owner,
        EmotionType.DIE, 0, (lastAttacker == null) ? 0 : lastAttacker.getObjectId()));
  ...
  public void onDialogRequest(Player player) {
    ...
    if (QuestEngine.getInstance()
        .onDialog(new QuestEnv((VisibleObject) getOwner(), player, Integer.valueOf(0), Integer.valueOf(-1)))) {
      return;
    }
  ...
  public void onDialogSelect(int dialogId, final Player player, int questId) {
    ...
    if (QuestEngine.getInstance()
        .onDialog(new QuestEnv((VisibleObject) npc, player, Integer.valueOf(questId), Integer.valueOf(dialogId))))
      return;
    switch (dialogId) {
      ...
      case 2:
        ...
        if (tradeListTemplate == null) {
          ...
          PacketSendUtility.sendPacket(player,
              (AionServerPacket) new SM_TRADELIST(npc,
      ...
      case 3:
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_SELL_ITEM(targetObjectId, player.getPrices().getVendorSellModifier()));
      ...
      case 29:
        ...
        if (player.getCommonData().getExpRecoverable() > 0L) {
          ...
          if (result) {
            PacketSendUtility.sendPacket(player,
                (AionServerPacket) new SM_QUESTION_WINDOW(160011, 0, new Object[] { String.valueOf(price) }));
      ...
      case 53:
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_MESSAGE(0, null, "This feature is not available yet", ChatType.ANNOUNCEMENTS));
  ...
  public void onAttack(Creature creature, int skillId, SM_ATTACK_STATUS.TYPE type, int damage) {
    ...
    if (actingCreature instanceof Player && QuestEngine.getInstance()
        .onAttack(new QuestEnv((VisibleObject) npc, (Player) actingCreature, Integer.valueOf(0), Integer.valueOf(0)))) {
      return;
    }
    AI<?> ai = npc.getAi();
    ...
    PacketSendUtility.broadcastPacket((VisibleObject) npc,
        (AionServerPacket) new SM_ATTACK_STATUS((Creature) npc, type, skillId, damage));
  }
  ...
  public void attackTarget(Creature target) {
    ...
    int attackType = 0;
    PacketSendUtility.broadcastPacket((VisibleObject) npc, (AionServerPacket) new SM_ATTACK((Creature) npc, target,
