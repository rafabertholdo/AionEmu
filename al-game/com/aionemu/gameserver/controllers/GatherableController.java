...
public class GatherableController extends VisibleObjectController<Gatherable> {
  ...
  public void rewardPlayer(Player player) {
    if (player != null) {
      ...
      int xpReward = (int) ((0.008D * (skillLvl + 100) * (skillLvl + 100) + 60.0D)
      ...
      if (player.getSkillList().addSkillXp(player, getOwner().getObjectTemplate().getHarvestSkill(), xpReward)) {
        ...
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) SM_SYSTEM_MESSAGE.MSG_DONT_GET_PRODUCTION_EXP(new DescriptionId(DataManager.SKILL_DATA
