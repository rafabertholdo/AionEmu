  ...
  extends CreatureController<Player>
{
  ...
  public void see(VisibleObject object) {
    ...
    if (object instanceof Player) {
      ...
      PacketSendUtility.sendPacket(getOwner(), (AionServerPacket)new SM_PLAYER_INFO((Player)object, getOwner().isEnemyPlayer((Player)object)));
      ...
      for (Iterator<Integer> i$ = QuestEngine.getInstance().getNpcQuestData(npc.getNpcId()).getOnQuestStart().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
        
        if (QuestService.checkStartCondition(new QuestEnv(object, getOwner(), Integer.valueOf(questId), Integer.valueOf(0))))
        {
    ...
    else if (object instanceof com.aionemu.gameserver.model.gameobjects.Gatherable || object instanceof com.aionemu.gameserver.model.gameobjects.StaticObject) {
  ...
  public void notSee(VisibleObject object, boolean isOutOfRange) {
    ...
    if (object instanceof Npc) {
      ...
      boolean update = false;
      for (Iterator<Integer> i$ = QuestEngine.getInstance().getNpcQuestData(((Npc)object).getNpcId()).getOnQuestStart().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
    ...
    PacketSendUtility.sendPacket(getOwner(), (AionServerPacket)new SM_DELETE((AionObject)object, isOutOfRange ? 0 : 15));
  }
  ...
  public void updateNearbyQuests() {
    ...
    for (VisibleObject obj : getOwner().getKnownList().getKnownObjects().values()) {
      ...
      {
        for (Iterator<Integer> i$ = QuestEngine.getInstance().getNpcQuestData(((Npc)obj).getNpcId()).getOnQuestStart().iterator(); i$.hasNext(); ) { int questId = ((Integer)i$.next()).intValue();
          
          if (QuestService.checkStartCondition(new QuestEnv(obj, getOwner(), Integer.valueOf(questId), Integer.valueOf(0))))
          {
            if (!getOwner().getNearbyQuests().contains(Integer.valueOf(questId)))
            {
              getOwner().getNearbyQuests().add(Integer.valueOf(questId));
  ...
  public void onEnterZone(ZoneInstance zoneInstance) {
    QuestEngine.getInstance().onEnterZone(new QuestEnv(null, getOwner(), Integer.valueOf(0), Integer.valueOf(0)), zoneInstance.getTemplate().getName());
  }
  ...
  public void onDie(Creature lastAttacker) {
    ...
    PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_EMOTION((Creature)player, EmotionType.DIE, 0, (lastAttacker == null) ? 0 : lastAttacker.getObjectId()), true);
    ...
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_DIE(hasSelfRezEffect, hasSelfRezItem, kiskTimeRemaining));
  ...
  public void attackTarget(Creature target) {
    ...
    int attackType = 0;
    PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_ATTACK((Creature)player, target, gameStats.getAttackCounter(), (int)time, attackType, attackResult), true);
  ...
  public void onAttack(Creature creature, int skillId, SM_ATTACK_STATUS.TYPE type, int damage) {
    ...
    PacketSendUtility.broadcastPacket(getOwner(), (AionServerPacket)new SM_ATTACK_STATUS((Creature)getOwner(), type, skillId, damage), true);
  }
  ...
  public void upgradePlayer(int level) {
    ...
    PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_LEVEL_UPDATE(player.getObjectId(), 0, level), true);
  ...
  public void createSummon(int npcId, int skillLvl) {
    ...
    PacketSendUtility.broadcastPacket((VisibleObject)summon, (AionServerPacket)new SM_EMOTION((Creature)summon, EmotionType.START_EMOTE2));
