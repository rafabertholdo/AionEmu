  ...
  extends NpcController
{
  ...
  private void bindHere(Player player) {
        ...
        public void acceptRequest(Creature requester, Player responder)
        {
          if (responder.getCommonData().getBindPoint() != BindpointController.this.bindPointTemplate.getBindId())
          {
            if (ItemService.decreaseKinah(responder, BindpointController.this.bindPointTemplate.getPrice())) {
              ...
              PacketSendUtility.broadcastPacket(responder, (AionServerPacket)new SM_LEVEL_UPDATE(responder.getObjectId(), 2, responder.getCommonData().getLevel()), true);
              PacketSendUtility.sendPacket(responder, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_DEATH_REGISTER_RESURRECT_POINT());
            }
            else {
              
              PacketSendUtility.sendPacket(responder, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_CANNOT_REGISTER_RESURRECT_POINT_NOT_ENOUGH_FEE());
              return;
            } 
          }
        }
    ...
    if (requested) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_QUESTION_WINDOW(160012, 0, new Object[] { price }));
    } 
  }
  ...
  public void onDialogRequest(Player player) {
    ...
    if (player.getCommonData().getBindPoint() == this.bindPointTemplate.getBindId()) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_ALREADY_REGISTER_THIS_RESURRECT_POINT());
    ...
    if (!CustomConfig.ENABLE_CROSS_FACTION_BINDING) {
      ...
      if (worldType == WorldType.ASMODAE && player.getCommonData().getRace() == Race.ELYOS) {
        ...
        PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_MESSAGE(0, null, "Elyos cannot bind in Asmodian territory.", ChatType.ANNOUNCEMENTS));
        
        return;
      } 
      if (worldType == WorldType.ELYSEA && player.getCommonData().getRace() == Race.ASMODIANS) {
        
        PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_MESSAGE(0, null, "Asmodians cannot bind in Elyos territory.", ChatType.ANNOUNCEMENTS));
        return;
      } 
    ...
    if (worldType == WorldType.PRISON) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_MESSAGE(0, null, "You cannot bind here.", ChatType.ANNOUNCEMENTS));
