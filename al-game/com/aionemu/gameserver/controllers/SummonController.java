...
public class SummonController extends CreatureController<Summon> {
  ...
  public void attackMode() {
    ...
    PacketSendUtility.sendPacket(master,
        (AionServerPacket) SM_SYSTEM_MESSAGE.SUMMON_ATTACKMODE(getOwner().getNameId()));
    PacketSendUtility.sendPacket(master, (AionServerPacket) new SM_SUMMON_UPDATE(getOwner()));
  ...
  public void attackTarget(Creature target) {
    ...
    int attackType = 0;
    PacketSendUtility.broadcastPacket((VisibleObject) summon, (AionServerPacket) new SM_ATTACK((Creature) summon,
        target, summon.getGameStats().getAttackCounter(), 274, attackType, attackList));
  ...
  public void onAttack(Creature creature, int skillId, SM_ATTACK_STATUS.TYPE type, int damage) {
    ...
    PacketSendUtility.broadcastPacket((VisibleObject) getOwner(),
        (AionServerPacket) new SM_ATTACK_STATUS((Creature) getOwner(), SM_ATTACK_STATUS.TYPE.REGULAR, 0, damage));
