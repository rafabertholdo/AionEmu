...
public class CraftSkillUpdateService
{
  ...
  public void learnSkill(Player player, Npc npc) {
    ...
    if (price > kinahItem.getItemCount()) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_MESSAGE(0, null, "You don't have enough Kinah.", ChatType.ANNOUNCEMENTS));
      return;
    } 
    ...
    if (result)
    {
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_QUESTION_WINDOW(900852, 0, new Object[] { new DescriptionId(DataManager.SKILL_DATA.getSkillTemplate(skillId).getNameId()), String.valueOf(price) }));
    }
  }
  ...
  private static boolean isCraftingSkill(int skillId) {
    for (Iterator<Integer> i$ = craftingSkillIds.iterator(); i$.hasNext(); ) { int i = ((Integer)i$.next()).intValue();
