...
public class ClassChangeService
{
  public static void showClassChangeDialog(Player player) {
    if (CustomConfig.ENABLE_SIMPLE_2NDCLASS) {
      ...
      if (player.getLevel() >= 9 && Arrays.<Integer>asList(new Integer[] { Integer.valueOf(0), Integer.valueOf(3), Integer.valueOf(6), Integer.valueOf(9) }).contains(Integer.valueOf(playerClass.ordinal())))
      {
  ...
  private static void addCompliteQuest(Player player, int questId) {
    ...
    if (qs == null) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_QUEST_ACCEPTED(questId, QuestStatus.COMPLETE.value(), 0));
    }
    else {
