...
public class MailService {
  ...
  public void sendMail(Player sender, String recipientName, String title, String message, int attachedItemObjId,
    ...
    PlayerCommonData recipientCommonData = ((PlayerDAO) DAOManager.getDAO(PlayerDAO.class))
    ...
    if (recipientCommonData.isOnline()) {
      ...
      if (!onlineRecipient.getMailbox().haveFreeSlots()) {
        ...
        PacketSendUtility.sendPacket(sender,
            (AionServerPacket) new SM_MAIL_SERVICE(MailMessage.RECIPIENT_MAILBOX_FULL));
      ...
      if (senderItem != null) {
        ...
        if (senderItem.getItemCount() == attachedItemCount) {
          ...
          itemMailCommission = Math.round(
              (float) (senderItem.getItemTemplate().getPrice() * attachedItem.getItemCount()) * qualityPriceRate);
          ...
          itemMailCommission = Math.round(
              (float) (attachedItem.getItemTemplate().getPrice() * attachedItem.getItemCount()) * qualityPriceRate);
        }
      }
    }
    ...
    if (attachedItem != null && !((InventoryDAO) DAOManager.getDAO(InventoryDAO.class)).store(attachedItem)) {
      return;
    }
    ...
    if (onlineRecipient != null) {
      ...
      PacketSendUtility.sendPacket(onlineRecipient,
          (AionServerPacket) new SM_MAIL_SERVICE(onlineRecipient, onlineRecipient.getMailbox().getLetters()));
  ...
  public void readMail(Player player, int letterId) {
    ...
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) new SM_MAIL_SERVICE(player, letter, letter.getTimeStamp().getTime()));
  ...
  public void getAttachments(Player player, int letterId, int attachmentType) {
    ...
    switch (attachmentType) {
      ...
      case 0:
        ...
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_ADD_ITEMS(Collections.singletonList(inventoryItem)));
  ...
  private class MailLoadTask implements Runnable {
    ...
    public void run() {
      ...
      PacketSendUtility.sendPacket(this.player,
          (AionServerPacket) new SM_MAIL_SERVICE(this.player, this.player.getMailbox().getLetters()));
