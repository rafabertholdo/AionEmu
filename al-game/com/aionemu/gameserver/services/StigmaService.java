...
public class StigmaService
{
  ...
  public static boolean notifyEquipAction(Player player, Item resultItem) {
    if (resultItem.getItemTemplate().isStigma()) {
      ...
      for (RequireSkill rs : stigmaInfo.getRequireSkill()) {
        ...
        Iterator<Integer> i$ = rs.getSkillId().iterator(); if (i$.hasNext()) { int id = ((Integer)i$.next()).intValue();
  ...
  public static boolean notifyUnequipAction(Player player, Item resultItem) {
    if (resultItem.getItemTemplate().isStigma()) {
      ...
      for (Item item : player.getEquipment().getEquippedItemsStigma()) {
        ...
        for (RequireSkill rs : si.getRequireSkill()) {
          ...
          if (rs.getSkillId().contains(Integer.valueOf(skillId))) {
            ...
            PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_SYSTEM_MESSAGE(1300410, new Object[] { new DescriptionId(resultItem.getItemTemplate().getNameId()), new DescriptionId(item.getItemTemplate().getNameId()) }));
            return false;
          } 
        } 
      } 
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_SYSTEM_MESSAGE(1300403, new Object[] { new DescriptionId(nameId) }));
    } 
    return true;
  }
    ...
    for (Item item : equippedItems) {
      ...
      if (item.getItemTemplate().isStigma()) {
        ...
        for (RequireSkill rs : stigmaInfo.getRequireSkill()) {
          ...
          for (Iterator<Integer> i$ = rs.getSkillId().iterator(); i$.hasNext(); ) { int id = ((Integer)i$.next()).intValue();
