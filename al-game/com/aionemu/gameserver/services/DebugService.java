  ...
  private void analyzeWorldPlayers() {
    ...
    for (Player player : World.getInstance().getAllPlayers()) {
      ...
      if (connection == null) {
        ...
        log.warn(String.format("[DEBUG SERVICE] Player without connection: detected: ObjId %d, Name %s, Spawned %s", new Object[] { Integer.valueOf(player.getObjectId()), player.getName(), Boolean.valueOf(player.isSpawned()) }));
      ...
      {
        log.warn(String.format("[DEBUG SERVICE] Player with large ping interval: ObjId %d, Name %s, Spawned %s, PingMS %d", new Object[] { Integer.valueOf(player.getObjectId()), player.getName(), Boolean.valueOf(player.isSpawned()), Long.valueOf(pingInterval) }));
      }
    } 
  }
