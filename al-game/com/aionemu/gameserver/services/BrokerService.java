...
public class BrokerService
{
  ...
  private Map<Integer, BrokerPlayerCache> playerBrokerCache = (Map<Integer, BrokerPlayerCache>)(new FastMap()).shared();
  ...
  public void showRequestedItems(Player player, int clientMask, int sortType, int startPage) {
    ...
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_BROKER_ITEMS(searchItems, totalSearchItemsCount, startPage));
  }
  ...
  public void registerItem(Player player, int itemUniqueId, int price) {
    ...
    if (player.getInventory().getKinahItem().getItemCount() < registrationCommition) {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_BROKER_REGISTRATION_SERVICE(BrokerMessages.NO_ENOUGHT_KINAH.getId()));
      ...
      if (item.getItem() != null) {
        ...
        if (resultItem != null) {
          ...
          if (result) {
            ...
            item.setPersistentState(PersistentState.DELETED);
            this.saveManager.add(new BrokerOpSaveTask(item));
            PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_ADD_ITEMS(Collections.singletonList(resultItem)));
          } 
