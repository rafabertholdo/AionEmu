...
public class TradeService
{
    ...
    for (TradeItem tradeItem : tradeList.getTradeItems()) {
      ...
      if (!ItemService.addItem(player, tradeItem.getItemTemplate().getTemplateId(), tradeItem.getCount())) {
        ...
        if (OptionsConfig.LOG_AUDIT) {
          log.warn(String.format("[AUDIT] Itemservice couldnt add all items on buy: %d %d %d %d", new Object[] { Integer.valueOf(player.getObjectId()), Integer.valueOf(tradeItem.getItemTemplate().getTemplateId()), Long.valueOf(tradeItem.getCount()), Long.valueOf(tradeItem.getCount()) }));
        }
  ...
  public static boolean performBuyFromAbyssShop(Player player, TradeList tradeList) {
    ...
    {
      ItemService.decreaseItemCountByItemId(player, itemId.intValue(), ((Integer)requiredItems.get(itemId)).intValue());
    }
