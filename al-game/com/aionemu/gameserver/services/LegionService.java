...
public class LegionService
{
  ...
  private void storeLegion(Legion legion, boolean newLegion) {
    if (newLegion) {
      ...
      ((LegionDAO)DAOManager.getDAO(LegionDAO.class)).storeLegionEmblem(legion.getLegionId(), legion.getLegionEmblem());
    } 
  }
  ...
  private void storeLegionMember(LegionMember legionMember, boolean newMember) {
    if (newMember) {
      ...
      ((LegionMemberDAO)DAOManager.getDAO(LegionMemberDAO.class)).storeLegionMember(legionMember.getObjectId(), legionMember);
    } 
  }
  ...
  private void loadLegionInfo(Legion legion) {
    ...
    legion.setLegionMembers(((LegionMemberDAO)DAOManager.getDAO(LegionMemberDAO.class)).loadLegionMembers(legion.getLegionId()));



    
    legion.setAnnouncementList(((LegionDAO)DAOManager.getDAO(LegionDAO.class)).loadAnnouncementList(legion.getLegionId()));
    ...
    if (this.legionRanking == null) {
      ...
      ThreadPoolManager.getInstance().scheduleAtFixedRate(new LegionRankingUpdateTask(), DELAY_LEGIONRANKING, DELAY_LEGIONRANKING);
  ...
  public LegionMember getLegionMember(int playerObjId) {
    ...
    if (legionMember != null && 
      checkDisband(legionMember.getLegion())) {
      return null;
    }
    return legionMember;
  }
  ...
  private LegionMemberEx getLegionMemberEx(int playerObjId) {
    ...
    LegionMemberEx legionMember = ((LegionMemberDAO)DAOManager.getDAO(LegionMemberDAO.class)).loadLegionMemberEx(playerObjId);
  ...
  private LegionMemberEx getLegionMemberEx(String playerName) {
    ...
    LegionMemberEx legionMember = ((LegionMemberDAO)DAOManager.getDAO(LegionMemberDAO.class)).loadLegionMemberEx(playerName);
    addCachedLegionMemberEx(legionMember);
    return legionMember;
  }
  ...
  public void createLegion(Player activePlayer, String legionName) {
    if (this.legionRestrictions.canCreateLegion(activePlayer, legionName)) {
      ...
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CREATED(legion.getLegionName()));
    } 
  }
  ...
  private void invitePlayerToLegion(final Player activePlayer, final Player targetPlayer) {
    if (this.legionRestrictions.canInvitePlayer(activePlayer, targetPlayer)) {
      ...
      RequestResponseHandler responseHandler = new RequestResponseHandler((Creature)activePlayer)
        {
          ...
          public void acceptRequest(Creature requester, Player responder)
          {
            if (!targetPlayer.getCommonData().isOnline()) {
              ...
              if (legion.addLegionMember(playerObjId)) {
                ...
                PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.NEW_MEMBER_JOINED(targetPlayer.getName()));
                ...
                PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CAN_NOT_ADD_MEMBER_ANY_MORE());
          ...
          public void denyRequest(Creature requester, Player responder) {
            PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.REJECTED_INVITE_REQUEST(targetPlayer.getName()));
          }
        };
      ...
      if (!requested) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.SEND_INVITE_REQUEST(targetPlayer.getName()));


        
        PacketSendUtility.sendPacket(targetPlayer, (AionServerPacket)new SM_QUESTION_WINDOW(80001, 0, new Object[] { legion.getLegionName(), legion.getLegionLevel() + "", activePlayer.getName() }));
      } 
    } 
  }
  ...
  private void displayLegionMessage(Player targetPlayer, Map.Entry<Timestamp, String> currentAnnouncement) {
    ...
    {
      PacketSendUtility.sendPacket(targetPlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_DISPLAY_ANNOUNCEMENT(currentAnnouncement.getValue(), (int)(((Timestamp)currentAnnouncement.getKey()).getTime() / 1000L), 2));
    }
  }
  ...
  private void appointBrigadeGeneral(final Player activePlayer, final Player targetPlayer) {
    if (this.legionRestrictions.canAppointBrigadeGeneral(activePlayer, targetPlayer)) {
      ...
      RequestResponseHandler responseHandler = new RequestResponseHandler((Creature)activePlayer)
        {
          ...
          public void acceptRequest(Creature requester, Player responder)
          {
            if (!targetPlayer.getCommonData().isOnline()) {
              ...
              PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MASTER_NO_SUCH_USER());
              ...
              if (legionMember.getRank().getRankId() > LegionRank.BRIGADE_GENERAL.getRankId()) {
                ...
                PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(activePlayer, 0, ""));


                
                legionMember.setRank(LegionRank.BRIGADE_GENERAL);
                PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(targetPlayer, 1300273, targetPlayer.getName()));
          ...
          public void denyRequest(Creature requester, Player responder) {
            PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MASTER_HE_DECLINE_YOUR_OFFER(targetPlayer.getName()));
          }
        };
      ...
      if (!requested) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MASTER_SENT_CANT_OFFER_WHEN_HE_IS_QUESTION_ASKED());
      
      }
      else {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MASTER_SENT_OFFER_MSG_TO_HIM(targetPlayer.getName()));



        
        PacketSendUtility.sendPacket(targetPlayer, (AionServerPacket)new SM_QUESTION_WINDOW(80011, activePlayer.getObjectId(), new Object[0]));
      } 
    } 
  }
  ...
  private void appointRank(Player activePlayer, Player targetPlayer, int rank) {
    if (this.legionRestrictions.canAppointRank(activePlayer, targetPlayer)) {
      ...
      PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(targetPlayer, msgId, targetPlayer.getName()));
    } 
  }
  ...
  private void changeSelfIntro(Player activePlayer, String newSelfIntro) {
    if (this.legionRestrictions.canChangeSelfIntro(activePlayer, newSelfIntro)) {
      ...
      PacketSendUtility.broadcastPacketToLegion(legionMember.getLegion(), (AionServerPacket)new SM_LEGION_UPDATE_SELF_INTRO(activePlayer.getObjectId(), newSelfIntro));
  ...
  private void changeNickname(Player activePlayer, Player targetPlayer, String newNickname) {
    ...
    if (this.legionRestrictions.canChangeNickname(legion, targetPlayer.getObjectId(), newNickname)) {
      ...
      PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_UPDATE_NICKNAME(targetPlayer.getObjectId(), newNickname));
    } 
  }
  ...
  private void updateAfterDisbandLegion(Legion legion) {
    for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
      ...
      PacketSendUtility.broadcastPacket(onlineLegionMember, (AionServerPacket)new SM_LEGION_UPDATE_TITLE(onlineLegionMember.getObjectId(), 0, "", 0), true);
      
      PacketSendUtility.sendPacket(onlineLegionMember, (AionServerPacket)new SM_LEGION_LEAVE_MEMBER(1300302, 0, legion.getLegionName()));
  ...
  private void updateMembersEmblem(Legion legion) {
    ...
    for (Player onlineLegionMember : legion.getOnlineLegionMembers())
    {
      PacketSendUtility.broadcastPacket(onlineLegionMember, (AionServerPacket)new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b()), true);
    }
  }
  ...
  private void updateMembersOfDisbandLegion(Legion legion, int unixTime) {
    for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
      ...
      PacketSendUtility.sendPacket(onlineLegionMember, (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(onlineLegionMember, 1300303, unixTime + ""));
  ...
  private void updateMembersOfRecreateLegion(Legion legion) {
    for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
      ...
      PacketSendUtility.sendPacket(onlineLegionMember, (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(onlineLegionMember, 1300307, ""));
  ...
  public void openLegionWarehouse(Player activePlayer) {
    if (this.legionRestrictions.canOpenWarehouse(activePlayer)) {
      ...
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_DIALOG_WINDOW(activePlayer.getObjectId(), 25));
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_WAREHOUSE_INFO(activePlayer.getLegion().getLegionWarehouse().getStorageItems(), StorageType.LEGION_WAREHOUSE.getId(), 0, true));
      
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_WAREHOUSE_INFO(null, StorageType.LEGION_WAREHOUSE.getId(), 0, false));
    } 
  }
    ...
    public void run() {
      ...
      for (Legion legion : legions) {
        ...
        try {
          ...
          {
            legion.setLegionRank(((Integer)LegionService.this.legionRanking.get(Integer.valueOf(legion.getLegionId()))).intValue());
  ...
  public void updateMemberInfo(Player player) {
    PacketSendUtility.broadcastPacketToLegion(player.getLegion(), (AionServerPacket)new SM_LEGION_UPDATE_MEMBER(player, 0, ""));
  }
  ...
  public void setLegionName(Legion legion, String newLegionName, boolean save) {
    ...
    {
      PacketSendUtility.broadcastPacket(legionMember, (AionServerPacket)new SM_LEGION_UPDATE_TITLE(legionMember.getObjectId(), legion.getLegionId(), legion.getLegionName(), legionMember.getLegionMember().getRank().getRankId()), true);
    }
  ...
  private void changeAnnouncement(Player activePlayer, String announcement) {
    if (this.legionRestrictions.canChangeAnnouncement(activePlayer.getLegionMember(), announcement)) {
      ...
      PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_EDIT(5, (int)(System.currentTimeMillis() / 1000L), announcement));
    } 
  }
  ...
  private void addLegionMember(Legion legion, Player player, LegionRank rank) {
    ...
    PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)new SM_LEGION_ADD_MEMBER(player, false, 0, ""));

    
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_LEGION_MEMBERLIST(loadLegionMemberExList(legion)));

    
    PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_LEGION_UPDATE_TITLE(player.getObjectId(), legion.getLegionId(), legion.getLegionName(), player.getLegionMember().getRank().getRankId()), true);


    
    LegionEmblem legionEmblem = legion.getLegionEmblem();
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b()));
  ...
  private boolean removeLegionMember(String charName, boolean kick, String playerName) {
    ...
    if (player != null)
    {
      PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_LEGION_UPDATE_TITLE(player.getObjectId(), 0, "", 2), true);
    }
    ...
    if (kick) {
      ...
      PacketSendUtility.broadcastPacketToLegion(legionMember.getLegion(), (AionServerPacket)new SM_LEGION_LEAVE_MEMBER(1300247, legionMember.getObjectId(), playerName, legionMember.getName()));
    
    }
    else {
      
      PacketSendUtility.broadcastPacketToLegion(legionMember.getLegion(), (AionServerPacket)new SM_LEGION_LEAVE_MEMBER(900699, legionMember.getObjectId(), charName));
    } 
  ...
  public void handleCharNameRequest(int exOpcode, Player activePlayer, String charName, String newNickname, int rank) {
    ...
    switch (exOpcode) {
      ...
      case 1:
        if (targetPlayer != null) {
          ...
          if (targetPlayer.getPlayerSettings().isInDeniedStatus(DeniedStatus.GUILD)) {
            ...
            PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_MSG_REJECTED_INVITE_GUILD(charName));
        ...
        if (this.legionRestrictions.canKickPlayer(activePlayer, charName))
        {
          if (removeLegionMember(charName, true, activePlayer.getName()))
          {
            ...
            if (targetPlayer != null) {
              ...
              PacketSendUtility.sendPacket(targetPlayer, (AionServerPacket)new SM_LEGION_LEAVE_MEMBER(1300246, 0, legion.getLegionName()));
      ...
      case 6:
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MEMBER_RANK_NO_USER());
        break;
      ...
      case 2:
        if (this.legionRestrictions.canLeave(activePlayer))
        {
          if (removeLegionMember(activePlayer.getName(), false, "")) {
            ...
            Legion legion = activePlayer.getLegion();
            PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_LEGION_LEAVE_MEMBER(1300241, 0, legion.getLegionName()));
  ...
  public void onLogin(Player activePlayer) {
    ...
    PacketSendUtility.broadcastPacketToLegion(legion, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_MSG_NOTIFY_LOGIN_GUILD(activePlayer.getName()), activePlayer.getObjectId());
    ...
    PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_LEGION_MEMBERLIST(loadLegionMemberExList(legion)));
  ...
  private class LegionRestrictions
  {
    ...
    private boolean canCreateLegion(Player activePlayer, String legionName) {
      ...
      if (activePlayer.getInventory().getKinahItem().getItemCount() < LegionConfig.LEGION_CREATE_REQUIRED_KINAH) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CREATE_NOT_ENOUGH_KINAH());
        return false;
      } 
      return true;
    }
    ...
    private boolean canInvitePlayer(Player activePlayer, Player targetPlayer) {
      ...
      if (activePlayer.getLifeStats().isAlreadyDead()) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CANT_INVITE_WHILE_DEAD());
        return false;
      } 
      ...
      if (targetPlayer.isLegionMember()) {
        ...
        if (legion.isMember(targetPlayer.getObjectId())) {
          ...
          PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_HE_IS_MY_GUILD_MEMBER(targetPlayer.getName()));
        
        }
        else {
          
          PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_HE_IS_OTHER_GUILD_MEMBER(targetPlayer.getName()));
        } 
      ...
      if (activePlayer.getCommonData().getRace() != targetPlayer.getCommonData().getRace() && !LegionConfig.LEGION_INVITEOTHERFACTION)
      {
    ...
    private boolean canKickPlayer(Player activePlayer, String charName) {
      ...
      if (legionMember.isBrigadeGeneral()) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CANT_KICK_BRIGADE_GENERAL());
        return false;
      } 
    ...
    private boolean canAppointBrigadeGeneral(Player activePlayer, Player targetPlayer) {
      ...
      if (!isBrigadeGeneral(activePlayer)) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MEMBER_RANK_DONT_HAVE_RIGHT());
        
        return false;
      } 
      if (isSelf(activePlayer, targetPlayer.getObjectId())) {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MASTER_ERROR_SELF());
        return false;
      } 
      if (!legion.isMember(targetPlayer.getObjectId()))
      {
    ...
    private boolean canAppointRank(Player activePlayer, Player targetPlayer) {
      ...
      if (isSelf(activePlayer, targetPlayer.getObjectId())) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_MEMBER_RANK_ERROR_SELF());
        return false;
      } 
      if (!legion.isMember(targetPlayer.getObjectId()))
      {
    ...
    private boolean canChangeLevel(Player activePlayer, long kinahAmount) {
      ...
      if (legion.getLegionLevel() == 5) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_LEVEL_CANT_LEVEL_UP());
        return false;
      } 
      if (activePlayer.getInventory().getKinahItem().getItemCount() < legion.getKinahPrice()) {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_LEVEL_NOT_ENOUGH_KINAH());
        return false;
      } 
      if (!legion.hasRequiredMembers()) {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_LEVEL_NOT_ENOUGH_MEMBER());
        return false;
      } 
      if (legion.getContributionPoints() < levelContributionPrice) {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CHANGE_LEVEL_NOT_ENOUGH_POINT());
        return false;
      } 
      return true;
    }
    ...
    private boolean canDisbandLegion(Player activePlayer, Legion legion) {
      if (!isBrigadeGeneral(activePlayer)) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_DISPERSE_ONLY_MASTER_CAN_DISPERSE());
      ...
      if (legion.isDisbanding()) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_DISPERSE_ALREADY_REQUESTED());
        return false;
      } 
      if (legion.getLegionWarehouse().getStorageItems().size() > 0) {
        
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_DISPERSE_CANT_DISPERSE_GUILD_STORE_ITEM_IN_WAREHOUSE());
    ...
    private boolean canLeave(Player activePlayer) {
      if (isBrigadeGeneral(activePlayer)) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_CANT_LEAVE_BEFORE_CHANGE_MASTER());
        return false;
      } 
      return true;
    }
    ...
    public boolean canOpenWarehouse(Player activePlayer) {
      ...
      if (legion.isDisbanding()) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.LEGION_WAREHOUSE_CANT_USE_WHILE_DISPERSE());
      ...
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_DIALOG_WINDOW(activePlayer.getObjectId(), 25));
      PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_WAREHOUSE_INFO(legion.getLegionWarehouse().getStorageItems(), StorageType.LEGION_WAREHOUSE.getId(), 0, true));
    ...
    public boolean canStoreLegionEmblem(Player activePlayer, int legionId, int emblemId) {
      ...
      if (activePlayer.getInventory().getKinahItem().getItemCount() < LegionConfig.LEGION_EMBLEM_REQUIRED_KINAH) {
        ...
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)SM_SYSTEM_MESSAGE.NOT_ENOUGH_KINAH(LegionConfig.LEGION_EMBLEM_REQUIRED_KINAH));
