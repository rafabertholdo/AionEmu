...
public class PrivateStoreService
{
  ...
  private static void createStore(Player activePlayer) {
    ...
    PacketSendUtility.broadcastPacket(activePlayer, (AionServerPacket)new SM_EMOTION((Creature)activePlayer, EmotionType.OPEN_PRIVATESHOP, 0, 0), true);
  ...
  public static void closePrivateStore(Player activePlayer) {
    ...
    PacketSendUtility.broadcastPacket(activePlayer, (AionServerPacket)new SM_EMOTION((Creature)activePlayer, EmotionType.CLOSE_PRIVATESHOP, 0, 0), true);
  }
  ...
  private static TradeList loadObjIds(Player seller, TradeList tradeList) {
    ...
    for (TradeItem tradeItem : tradeList.getTradeItems()) {
      ...
      int i = 0;
      for (Iterator<Integer> i$ = store.getSoldItems().keySet().iterator(); i$.hasNext(); ) { int itemObjId = ((Integer)i$.next()).intValue();
  ...
  public static void openPrivateStore(Player activePlayer, String name) {
    if (name != null) {
      ...
      PacketSendUtility.broadcastPacket(activePlayer, (AionServerPacket)new SM_PRIVATE_STORE_NAME(activePlayer.getObjectId(), name), true);
    
    }
    else {
      
      PacketSendUtility.broadcastPacket(activePlayer, (AionServerPacket)new SM_PRIVATE_STORE_NAME(activePlayer.getObjectId(), ""), true);
    } 
  }
}
