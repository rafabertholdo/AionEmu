...
public class AllianceService
{
    ...
    if (RestrictionsManager.canInviteToAlliance(inviter, invited)) {
      ...
      if (result) {
        ...
        if (invited.isInGroup()) {
          ...
          PacketSendUtility.sendPacket(inviter, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_PARTY_ALLIANCE_INVITED_HIS_PARTY(invited.getName()));
        }
        else {
          
          PacketSendUtility.sendPacket(inviter, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_INVITED_HIM(invited.getName()));
        } 
        
        PacketSendUtility.sendPacket(invited, (AionServerPacket)new SM_QUESTION_WINDOW(70004, 0, new Object[] { inviter.getName() }));
      } 
    } 
  }
    ...
    RequestResponseHandler responseHandler = new RequestResponseHandler((Creature)inviter)
      {
        ...
        public void denyRequest(Creature requester, Player responder) {
          PacketSendUtility.sendPacket(inviter, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_PARTY_ALLIANCE_HE_REJECT_INVITATION(responder.getName()));
        }
      };
      ...
      switch (event) {
        ...
        case ENTER:
          if (!member.equals(memberToUpdate.getPlayer())) {
            PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_HE_ENTERED_FORCE(memberToUpdate.getName()));
          }
        case LEAVE:
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_PARTY_ALLIANCE_HE_LEAVED_PARTY(memberToUpdate.getName()));
        
        case LEAVE_TIMEOUT:
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_HE_BECOME_OFFLINE_TIMEOUT(memberToUpdate.getName()));
        
        case BANNED:
          if (member.equals(memberToUpdate.getPlayer())) {
            PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_BAN_ME(params[0])); continue;
          } 
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_BAN_HIM(params[0], memberToUpdate.getName()));
      } 
    } 
  }
    ...
    for (PlayerAllianceMember allianceMember : alliance.getMembers()) {
      ...
      switch (event) {
        ...
        case APPOINT_CAPTAIN:
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_CHANGE_LEADER(params[0], alliance.getCaptain().getName()));
        
        case APPOINT_VICE_CAPTAIN:
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_PROMOTE_MANAGER(params[0]));
        
        case DEMOTE_VICE_CAPTAIN:
          PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_DEMOTE_MANAGER(params[0]));
      } 
    } 
  }
  ...
  private void sendOtherMemberInfo(PlayerAlliance alliance, Player memberToSend) {
    for (PlayerAllianceMember allianceMember : alliance.getMembers()) {
      ...
      if (!allianceMember.isOnline() || 
        memberToSend.getObjectId() == allianceMember.getObjectId())
        continue;  PacketSendUtility.sendPacket(memberToSend, (AionServerPacket)new SM_ALLIANCE_MEMBER_INFO(allianceMember, PlayerAllianceEvent.UPDATE));
  ...
  public void playerStatusInfo(Player actingMember, int status, int playerObjId) {
    ...
    switch (status) {
      ...
      case 12:
        ...
        break;
      case 14:
        removeMemberFromAlliance(alliance, playerObjId, PlayerAllianceEvent.BANNED, new String[] { actingMember.getName() });
        break;
      case 15:
        oldLeader = alliance.getCaptain().getName();
        alliance.setLeader(playerObjId);
        broadcastAllianceInfo(alliance, PlayerAllianceEvent.APPOINT_CAPTAIN, new String[] { oldLeader, alliance.getCaptain().getName() });
        break;
      case 19:
      ...
      case 23:
        ...
        broadcastAllianceInfo(alliance, PlayerAllianceEvent.APPOINT_VICE_CAPTAIN, new String[] { alliance.getPlayer(playerObjId).getName() });
        break;
      case 24:
        alliance.demoteViceLeader(playerObjId);
        broadcastAllianceInfo(alliance, PlayerAllianceEvent.DEMOTE_VICE_CAPTAIN, new String[] { alliance.getPlayer(playerObjId).getName() });
        break;
    } 
  }
  ...
  public void showBrand(PlayerAlliance alliance, int brandId, int targetObjectId) {
    for (PlayerAllianceMember allianceMember : alliance.getMembers()) {
      ...
      if (!allianceMember.isOnline())
        continue;  PacketSendUtility.sendPacket(allianceMember.getPlayer(), (AionServerPacket)new SM_SHOW_BRAND(brandId, targetObjectId));
    } 
  }
  ...
  public void doReward(PlayerAlliance alliance, Monster owner) {
    ...
    if (worldType == WorldType.ABYSS)
    {
      ...
      apRewardPerMember = Math.round((StatFunctions.calculateGroupAPReward(highestLevel, (Creature)owner) / players.size()));
    }
    ...
    for (Player member : players) {
      ...
      QuestEngine.getInstance().onKill(new QuestEnv((VisibleObject)owner, member, Integer.valueOf(0), Integer.valueOf(0)));
    } 
