...
public class ItemService {
  ...
  public static void splitItem(Player player, int itemObjId, long splitAmount, int slotNum, int sourceStorageType,
    ...
    if (itemToSplit == null) {
      ...
      if (itemToSplit.getObjectId() != itemObjId || itemToSplit == null) {
        ...
        log.warn(String.format("CHECKPOINT: attempt to split null item %d %d %d",
            new Object[] { Integer.valueOf(itemObjId), Long.valueOf(splitAmount), Integer.valueOf(slotNum) }));
  ...
  public static boolean addItem(Player player, int itemId, long count) {
    ...
    if (OptionsConfig.LOG_ITEM)
      log.info(String.format("[ITEM] ID/Count - %d/%d to player %s.",
          new Object[] { Integer.valueOf(itemId), Long.valueOf(count), player.getName() }));
  ...
  public static void socketGodstone(Player player, int weaponId, int stoneId) {
    ...
    if (weaponItem == null) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_GIVE_ITEM_PROC_CANNOT_GIVE_PROC_TO_EQUIPPED_ITEM);
