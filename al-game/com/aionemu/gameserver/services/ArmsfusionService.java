...
public class ArmsfusionService {
  public static void fusionWeapons(Player player, int firstItemUniqueId, int secondItemUniqueId, int price) {
    ...
    if (firstItem == null || secondItem == null
        || !(player.getTarget() instanceof com.aionemu.gameserver.model.gameobjects.Npc)) {
      return;
    }
    ...
    if (secondItem.getItemTemplate().getLevel() > firstItem.getItemTemplate().getLevel()) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_COMPOUND_ERROR_MAIN_REQUIRE_HIGHER_LEVEL);
    ...
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) SM_SYSTEM_MESSAGE.STR_COMPOUND_SUCCESS(firstItem.getNameID(), secondItem.getNameID()));
  }
  ...
  public static void breakWeapons(Player player, int weaponToBreakUniqueId) {
    ...
    if (!weaponToBreak.hasFusionedItem()) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_DECOMPOUND_ERROR_NOT_AVAILABLE(weaponToBreak.getNameID()));
    ...
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) SM_SYSTEM_MESSAGE.STR_COMPOUNDED_ITEM_DECOMPOUND_SUCCESS(weaponToBreak.getNameID()));
  }
}
