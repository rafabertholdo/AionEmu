...
public class DropService {
  ...
  public void registerDrop(Npc npc, Player player, int lvl) {
    ...
    if (player.isInAlliance()) {
      ...
      ((DropNpc) this.dropRegistrationMap.get(Integer.valueOf(npcUniqueId)))
          .setGroupSize(player.getPlayerAlliance().size());
    } else if (player.isInGroup()) {

      this.dropRegistrationMap.put(Integer.valueOf(npcUniqueId),
          new DropNpc(GroupService.getInstance().getMembersToRegistrateByRules(player, player.getPlayerGroup(), npc)));
  ...
  public void requestDropList(Player player, int npcId) {
    ...
    PacketSendUtility.broadcastPacket(player,
        (AionServerPacket) new SM_EMOTION((Creature) player, EmotionType.START_LOOT, 0, npcId), true);
  }
  ...
  public void requestDropList(Player player, int npcId, boolean close) {
    ...
    PacketSendUtility.broadcastPacket(player,
        (AionServerPacket) new SM_EMOTION((Creature) player, EmotionType.END_LOOT, 0, npcId), true);
            ...
            for (Player member : dropNpc.getInRangePlayers()) {
              ...
              if (member.isOnline()) {
                ...
                PacketSendUtility.sendPacket(member,
                    (AionServerPacket) new SM_GROUP_LOOT(member.getPlayerGroup().getGroupId(), itemId, npcId,
                        dropNpc.getDistributionType()));
              }
            }
          ...
          if (dropNpc.getDistributionType() > 1 && !dropNpc.isInUse()) {
            ...
            for (PlayerAllianceMember allianceMember : player.getPlayerAlliance().getMembers()) {
              ...
              if (member.isOnline()) {
                ...
                dropNpc.addPlayerStatus(member);
                PacketSendUtility.sendPacket(member,
                    (AionServerPacket) new SM_GROUP_LOOT(member.getPlayerAlliance()
                        .getPlayerAllianceGroupForMember(member.getObjectId()).getAllianceId(), itemId, npcId,
                        dropNpc.getDistributionType()));
              }
            }
      ...
      if (requestedItem.isDistributeItem()) {
        ...
        if (requestedItem.getWinningPlayer().getInventory().isFull()) {
          ...
          PacketSendUtility.sendPacket(requestedItem.getWinningPlayer(),
              (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_DICE_INVEN_ERROR);
        ...
        if (player.isInGroup()) {
          for (Player member : ((DropNpc) this.dropRegistrationMap.get(Integer.valueOf(npcId))).getInRangePlayers()) {
            ...
            if (!player.equals(member))
              PacketSendUtility.sendPacket(member,
                  (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_DICE_GIVEUP_OTHER(player.getName()));
          }
        }
        if (player.isInAlliance()) {
          for (PlayerAllianceMember allianceMember : player.getPlayerAlliance().getMembers()) {
      ...
      case 1:
        ...
        if (player.isInGroup()) {
          for (Player member : ((DropNpc) this.dropRegistrationMap.get(Integer.valueOf(npcId))).getInRangePlayers()) {
            ...
            if (!player.equals(member))
              PacketSendUtility.sendPacket(member,
                  (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_DICE_RESULT_OTHER(player.getName(), luck));
          }
        }
        if (player.isInAlliance()) {
          for (PlayerAllianceMember allianceMember : player.getPlayerAlliance().getMembers()) {
  ...
  private void winningRollActions(Player player, int itemId, int npcId) {
    ...
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_LOOT_GET_ITEM_ME(new DescriptionId(nameId)));

    if (player.isInGroup()) {
      for (Player member : ((DropNpc) this.dropRegistrationMap.get(Integer.valueOf(npcId))).getInRangePlayers()) {

        if (!player.equals(member)) {
          PacketSendUtility.sendPacket(member, (AionServerPacket) SM_SYSTEM_MESSAGE
              .STR_MSG_LOOT_GET_ITEM_OTHER(player.getName(), new DescriptionId(nameId)));
        }
      }
    }
    if (player.isInAlliance()) {
    ...
    if (player.isInGroup()) {
      for (Player member : dropNpc.getInRangePlayers()) {
        ...
        if (!player.equals(member)) {
          ...
          PacketSendUtility.sendPacket(member,
              (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_PAY_ACCOUNT_OTHER(player.getName(), highestValue));
          long distributeKinah = highestValue / (dropNpc.getGroupSize() - 1);
          ItemService.increaseKinah(member, distributeKinah);
          PacketSendUtility.sendPacket(member, (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_PAY_DISTRIBUTE(highestValue,
