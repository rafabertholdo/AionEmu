...
public class CraftService
{
  ...
  public static void finishCrafting(Player player, RecipeTemplate recipetemplate, boolean critical) {
    ...
    if (productItemId != 0) {
      ...
      int xpReward = (int)((0.008D * (recipetemplate.getSkillpoint().intValue() + 100) * (recipetemplate.getSkillpoint().intValue() + 100) + 60.0D) * player.getRates().getCraftingXPRate());
      ...
      if (player.getSkillList().addSkillXp(player, recipetemplate.getSkillid().intValue(), xpReward)) {
        ...
        PacketSendUtility.sendPacket(player, (AionServerPacket)SM_SYSTEM_MESSAGE.MSG_DONT_GET_PRODUCTION_EXP(new DescriptionId(recipetemplate.getSkillid().intValue())));
      } 
  ...
  public static void startCrafting(Player player, int targetTemplateId, int recipeId, int targetObjId) {
    ...
    if (recipeTemplate != null) {
      ...
      for (Component component : recipeTemplate.getComponent()) {
        ...
        if (player.getInventory().getItemCountByItemId(component.getItemid().intValue()) < component.getQuantity().intValue()) {
      ...
      for (Component component : recipeTemplate.getComponent())
      {
        ItemService.decreaseItemCountByItemId(player, component.getItemid().intValue(), component.getQuantity().intValue());
      }
