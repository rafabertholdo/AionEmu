...
public class PlayerService {
  ...
  public static boolean storeNewPlayer(Player player, String accountName, int accountId) {
    return (((PlayerDAO) DAOManager.getDAO(PlayerDAO.class)).saveNewPlayer(player.getCommonData(), accountId,
        accountName) && ((PlayerAppearanceDAO) DAOManager.getDAO(PlayerAppearanceDAO.class)).store(player)
  ...
  public static Player getPlayer(int playerObjId, Account account) {
    ...
    MacroList macroses = ((PlayerMacrossesDAO) DAOManager.getDAO(PlayerMacrossesDAO.class))
        .restoreMacrosses(playerObjId);
    ...
    Storage inventory = ((InventoryDAO) DAOManager.getDAO(InventoryDAO.class)).loadStorage(player, player.getObjectId(),
        StorageType.CUBE);
    ItemService.loadItemStones(inventory.getStorageItems());

    player.setStorage(inventory, StorageType.CUBE);

    Storage warehouse = ((InventoryDAO) DAOManager.getDAO(InventoryDAO.class)).loadStorage(player, player.getObjectId(),
    ...
    if (player.getCommonData().getTitleId() > 0) {
      TitleChangeListener.onTitleChange((CreatureGameStats) player.getGameStats(), player.getCommonData().getTitleId(),
          true);
    }
  ...
  public static Player newPlayer(PlayerCommonData playerCommonData, PlayerAppearance playerAppearance) {
    ...
    PlayerInitialData.PlayerCreationData playerCreationData = playerInitialData
  ...
  public static void playerLoggedIn(Player player) {
    if (log.isDebugEnabled()) {
      log.debug(
          "Player logged in: " + player.getName() + " Account: " + player.getClientConnection().getAccount().getName());
    }
  ...
  public static void playerLoggedOut(Player player) {
    if (log.isDebugEnabled()) {
      log.debug("Player logged out: " + player.getName() + " Account: "
  ...
  private static void storeDeletionTime(PlayerAccountData accData) {
    ((PlayerDAO) DAOManager.getDAO(PlayerDAO.class)).updateDeletionTime(accData.getPlayerCommonData().getPlayerObjId(),
  ...
  public static void addMacro(Player player, int macroOrder, String macroXML) {
    if (player.getMacroList().addMacro(macroOrder, macroXML)) {
      ((PlayerMacrossesDAO) DAOManager.getDAO(PlayerMacrossesDAO.class)).addMacro(player.getObjectId(), macroOrder,
          macroXML);
    }
  }
