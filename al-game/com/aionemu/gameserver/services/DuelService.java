...
public class DuelService
{
  ...
  public void onDuelRequest(Player requester, Player responder) {
    ...
    PacketSendUtility.sendPacket(responder, (AionServerPacket)new SM_QUESTION_WINDOW(50028, 0, new Object[] { requester.getName() }));
  ...
  public void confirmDuelWith(Player requester, Player responder) {
    ...
    RequestResponseHandler rrh = new RequestResponseHandler((Creature)responder)
      {
        public void denyRequest(Creature requester, Player responder)
        {
          DuelService.log.debug("[Duel] Player " + responder.getName() + " confirmed his duel with " + requester.getName());
        }
    ...
    PacketSendUtility.sendPacket(requester, (AionServerPacket)new SM_QUESTION_WINDOW(50030, 0, new Object[] { responder.getName() }));
  ...
  private void rejectDuelRequest(Player requester, Player responder) {
    ...
    PacketSendUtility.sendPacket(responder, (AionServerPacket)SM_SYSTEM_MESSAGE.DUEL_REJECT_DUEL_OF(requester.getName()));
  }
  ...
  public void loseDuel(Player player) {
    ...
    if (opponent != null) {
      ...
      PacketSendUtility.sendPacket(opponent, (AionServerPacket)SM_DUEL.SM_DUEL_RESULT(DuelResult.DUEL_WON, player.getName()));
      PacketSendUtility.sendPacket(player, (AionServerPacket)SM_DUEL.SM_DUEL_RESULT(DuelResult.DUEL_LOST, opponent.getName()));
    }
    else {
  ...
  public boolean isDueling(int playerObjId) {
    return (this.duels.containsKey(Integer.valueOf(playerObjId)) && this.duels.containsValue(Integer.valueOf(playerObjId)));
  }
  ...
  public boolean isDueling(int playerObjId, int targetObjId) {
    return (this.duels.containsKey(Integer.valueOf(playerObjId)) && ((Integer)this.duels.get(Integer.valueOf(playerObjId))).intValue() == targetObjId);
  }
