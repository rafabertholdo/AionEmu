  ...
  public static void expandWarehouse(final Player player, Npc npc) {
    ...
    if (npcCanExpandLevel(expandTemplate, player.getWarehouseSize() + 1) && validateNewSize(player.getWarehouseSize() + 1))
    {
      ...
      if (validateNewSize(player.getWarehouseSize() + 1)) {
        ...
        if (result)
        {
          PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_QUESTION_WINDOW(900686, 0, new Object[] { String.valueOf(price) }));
        }
      } else {
  ...
  public static void sendWarehouseInfo(Player player, boolean sendAccountWh) {
    ...
    if (itemsSize != 0) {
      ...
      while (index + 10 < itemsSize) {
        ...
        PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_WAREHOUSE_INFO(items.subList(index, index + 10), StorageType.REGULAR_WAREHOUSE.getId(), whSize, firstPacket));
        
        index += 10;
        firstPacket = false;
      } 
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_WAREHOUSE_INFO(items.subList(index, itemsSize), StorageType.REGULAR_WAREHOUSE.getId(), whSize, firstPacket));
    } 

    
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_WAREHOUSE_INFO(null, StorageType.REGULAR_WAREHOUSE.getId(), whSize, false));
    ...
    if (sendAccountWh)
    {
      ...
      PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_WAREHOUSE_INFO(player.getStorage(StorageType.ACCOUNT_WAREHOUSE.getId()).getAllItems(), StorageType.ACCOUNT_WAREHOUSE.getId(), 0, true));
    }



    
    PacketSendUtility.sendPacket(player, (AionServerPacket)new SM_WAREHOUSE_INFO(null, StorageType.ACCOUNT_WAREHOUSE.getId(), 0, false));
  }
}
