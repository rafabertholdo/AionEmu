...
public class TeleportService {
  ...
  public static void flightTeleport(TeleporterTemplate template, int locId, Player player) {
    if (template.getTeleLocIdData() == null) {
      ...
      log.info(String.format("Missing locId for this teleporter at teleporter_templates.xml with locId: %d",
          new Object[] { Integer.valueOf(locId) }));
    ...
    if (locationTemplate == null) {
      ...
      log.info(String.format("Missing info at teleport_location.xml with locId: %d",
          new Object[] { Integer.valueOf(locId) }));
    ...
    PacketSendUtility.broadcastPacket(player, (AionServerPacket) new SM_EMOTION((Creature) player,
        EmotionType.START_FLYTELEPORT, location.getTeleportId(), 0), true);
  }
  ...
  public static void regularTeleport(TeleporterTemplate template, int locId, Player player) {
    ...
    PacketSendUtility.sendPacket(player, (AionServerPacket) new SM_TELEPORT_LOC(locationTemplate.getMapId(),
  ...
  public static void showMap(Player player, int targetObjectId, int npcId) {
    ...
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) new SM_TELEPORT_MAP(player, targetObjectId, getTeleporterTemplate(npcId)));
  }
  ...
  public static void changeChannel(Player player, int channel) {
    world.despawn((VisibleObject) player);
    world.setPosition((VisibleObject) player, player.getWorldId(), channel + 1, player.getX(), player.getY(),
  ...
  public static void moveToBindLocation(Player player, boolean useTeleport, int delay) {
    ...
    if (bindPointId != 0) {
      ...
      PlayerInitialData.LocationData locationData = DataManager.PLAYER_INITIAL_DATA
  ...
  public static void sendSetBindPoint(Player player) {
    ...
    if (player.getCommonData().getBindPoint() != 0) {
      ...
      BindPointTemplate bplist = DataManager.BIND_POINT_DATA
          .getBindPointTemplate2(player.getCommonData().getBindPoint());
      worldId = bplist.getZoneId();
      x = bplist.getX();
      y = bplist.getY();
      z = bplist.getZ();
