...
public class GroupService
{
  ...
  public void invitePlayerToGroup(final Player inviter, final Player invited) {
    if (RestrictionsManager.canInviteToGroup(inviter, invited)) {
      ...
      RequestResponseHandler responseHandler = new RequestResponseHandler((Creature)inviter)
        {
          public void acceptRequest(Creature requester, Player responder)
          {
            ...
            PacketSendUtility.sendPacket(inviter, (AionServerPacket)SM_SYSTEM_MESSAGE.REQUEST_GROUP_INVITE(invited.getName()));
            if (group != null) {
          ...
          public void denyRequest(Creature requester, Player responder) {
            PacketSendUtility.sendPacket(inviter, (AionServerPacket)SM_SYSTEM_MESSAGE.REJECT_GROUP_INVITE(responder.getName()));
          }
        };
      ...
      if (result)
      {
        PacketSendUtility.sendPacket(invited, (AionServerPacket)new SM_QUESTION_WINDOW(60000, 0, new Object[] { inviter.getName() }));
      }
    } 
  }
    ...
    if (rewardcount <= amount) {
      ...
      for (Player groupMember : pg.getMembers()) {
        ...
        if (groupMember.equals(player)) {
          ItemService.decreaseKinah(groupMember, amount); continue;
        } 
  ...
  public void onLogin(Player activePlayer) {
    ...
    for (Player member : group.getMembers()) {
      ...
      if (!activePlayer.equals(member)) {
        PacketSendUtility.sendPacket(activePlayer, (AionServerPacket)new SM_GROUP_MEMBER_INFO(group, member, GroupEvent.ENTER));
      }
    } 
  }
  ...
  public void scheduleRemove(final Player player) {
    ...
    for (Player groupMember : player.getPlayerGroup().getMembers())
    {
      ...
      PacketSendUtility.sendPacket(groupMember, (AionServerPacket)SM_SYSTEM_MESSAGE.PARTY_HE_BECOME_OFFLINE(player.getName()));
    }
  }
  ...
  public List<Integer> getGroupMembers(PlayerGroup group, boolean except) {
    List<Integer> luckyMembers = new ArrayList<Integer>();
    for (Iterator<Integer> i$ = group.getMemberObjIds().iterator(); i$.hasNext(); ) { int memberObjId = ((Integer)i$.next()).intValue();
