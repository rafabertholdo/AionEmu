...
public class Crypt {
  ...
  public final int enableKey() {
    ...
    this.clientPacketKey = new byte[] { (byte) (key & 0xFF), (byte) (key >> 8 & 0xFF), (byte) (key >> 16 & 0xFF),
        (byte) (key >> 24 & 0xFF), -95, 108, 84, -121 };
  ...
  public final boolean decrypt(ByteBuffer buf) {
    ...
    for (int i = 1; i < size; i++, arrayIndex++) {
      ...
      data[arrayIndex] = (byte) (data[arrayIndex] ^ staticKey[i & 0x3F] & 0xFF ^ this.clientPacketKey[i & 0x7] & 0xFF
          ^ prev);
      prev = curr;
    }

    long oldKey = (this.clientPacketKey[0] & 0xFFL) << 0L | (this.clientPacketKey[1] & 0xFFL) << 8L
        | (this.clientPacketKey[2] & 0xFFL) << 16L | (this.clientPacketKey[3] & 0xFFL) << 24L
        | (this.clientPacketKey[4] & 0xFFL) << 32L | (this.clientPacketKey[5] & 0xFFL) << 40L
        | (this.clientPacketKey[6] & 0xFFL) << 48L | (this.clientPacketKey[7] & 0xFFL) << 56L;
  ...
  public final void encrypt(ByteBuffer buf) {
    ...
    for (int i = 1; i < size; i++, arrayIndex++) {
      ...
      data[arrayIndex] = (byte) (data[arrayIndex] ^ staticKey[i & 0x3F] & 0xFF ^ this.serverPacketKey[i & 0x7] & 0xFF
          ^ prev);
      prev = data[arrayIndex];
    }

    long oldKey = (this.serverPacketKey[0] & 0xFFL) << 0L | (this.serverPacketKey[1] & 0xFFL) << 8L
        | (this.serverPacketKey[2] & 0xFFL) << 16L | (this.serverPacketKey[3] & 0xFFL) << 24L
        | (this.serverPacketKey[4] & 0xFFL) << 32L | (this.serverPacketKey[5] & 0xFFL) << 40L
        | (this.serverPacketKey[6] & 0xFFL) << 48L | (this.serverPacketKey[7] & 0xFFL) << 56L;
