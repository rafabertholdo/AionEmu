    ...
    switch (this.type) {
        ...
        if (!this.result) {
          ...
          if (admin != null)
            PacketSendUtility.sendMessage(admin,
                this.playerName + " has been promoted Administrator with role " + this.param);
          if (player != null) {
            PacketSendUtility.sendMessage(player,
                "You have been promoted Administrator with role " + this.param + " by " + this.adminName);
          }
      ...
      case 2:
        if (!this.result) {
          ...
          if (admin != null)
            PacketSendUtility.sendMessage(admin,
                this.playerName + " has been promoted membership with level " + this.param);
          if (player != null) {

            player.setRates(Rates.getRatesFor(this.param));
            PacketSendUtility.sendMessage(player,
                "You have been promoted membership with level " + this.param + " by " + this.adminName);
          }
