...
public class CM_CHAT_MESSAGE_PUBLIC extends AionClientPacket {
      ...
      switch (this.type) {
        ...
        case ALLIANCE:
          ...
          if (OptionsConfig.LOG_CHAT) {
            log.info(String.format("[MESSAGE] - A <%d>: [%s]> %s", new Object[] {
                Integer.valueOf(player.getPlayerAlliance().getObjectId()), player.getName(), this.message }));
          }
        ...
        case LEGION:
          if (OptionsConfig.LOG_CHAT) {
            log.info(String.format("[MESSAGE] - L <%s>: [%s]> %s",
                new Object[] { player.getLegion().getLegionName(), player.getName(), this.message }));
          }
  ...
  private void broadcastToNonBlockedPlayers(final Player player) {
    PacketSendUtility.broadcastPacket(player, (AionServerPacket) new SM_MESSAGE(player, this.message, this.type), true,
  ...
  private void broadcastToGroupMembers(Player player) {
    if (player.isInGroup()) {
      ...
      if (OptionsConfig.LOG_CHAT)
        log.info(String.format("[MESSAGE] - G <%d>: [%s]> %s",
            new Object[] { Integer.valueOf(player.getPlayerGroup().getGroupId()), player.getName(), this.message }));
      ...
      if (OptionsConfig.LOG_CHAT)
        log.info(String.format("[MESSAGE] - A <%d>: [%s]> %s", new Object[] {
            Integer.valueOf(player.getPlayerAlliance().getObjectId()), player.getName(), this.message }));
      PlayerAllianceGroup allianceGroup = player.getPlayerAlliance()
          .getPlayerAllianceGroupForMember(player.getObjectId());
        ...
        for (PlayerAllianceMember allianceMember : allianceGroup.getMembers()) {
          if (allianceMember.isOnline()) {
            PacketSendUtility.sendPacket(allianceMember.getPlayer(),
                (AionServerPacket) new SM_MESSAGE(player, this.message, this.type));
          }
        }
