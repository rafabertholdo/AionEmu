...
public class LsPacketHandlerFactory {
  ...
  private LsPacketHandlerFactory() {
    addPacket((LsClientPacket) new CM_ACCOUNT_RECONNECT_KEY(3),
        new LoginServerConnection.State[] { LoginServerConnection.State.AUTHED });
    addPacket((LsClientPacket) new CM_ACOUNT_AUTH_RESPONSE(1),
        new LoginServerConnection.State[] { LoginServerConnection.State.AUTHED });
    addPacket((LsClientPacket) new CM_GS_AUTH_RESPONSE(0),
        new LoginServerConnection.State[] { LoginServerConnection.State.CONNECTED });
    addPacket((LsClientPacket) new CM_REQUEST_KICK_ACCOUNT(2),
        new LoginServerConnection.State[] { LoginServerConnection.State.AUTHED });
    addPacket((LsClientPacket) new CM_LS_CONTROL_RESPONSE(4),
        new LoginServerConnection.State[] { LoginServerConnection.State.AUTHED });
    addPacket((LsClientPacket) new CM_BAN_RESPONSE(5),
        new LoginServerConnection.State[] { LoginServerConnection.State.AUTHED });
  }
