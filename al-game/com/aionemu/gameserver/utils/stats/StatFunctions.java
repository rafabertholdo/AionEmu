...
public class StatFunctions {
  ...
  public static int calculatePvPApLost(Player defeated, Player winner) {
    int pointsLost = Math
        .round(defeated.getAbyssRank().getRank().getPointsLost() * defeated.getRates().getApPlayerRate());
  ...
  public static int calculatePhysicDamageToTarget(Creature attacker, Creature target, int skillDamages) {
    ...
    if (attacker instanceof Player) {
      ...
      if (weaponType != null) {
        ...
        resultDamage = Math.round(base
            * (ags.getCurrentStat(StatEnum.POWER) * 0.01F + ags.getBaseStat(StatEnum.MAIN_HAND_POWER) * 0.2F * 0.01F)
  ...
  public static int calculateOffHandPhysicDamageToTarget(Creature attacker, Creature target) {
    ...
    int base = Rnd.get(min, max);
    Damage = Math.round(
        base * (ags.getCurrentStat(StatEnum.POWER) * 0.01F + ags.getBaseStat(StatEnum.OFF_HAND_POWER) * 0.2F * 0.01F)
  ...
  public static int calculateMagicDamageToTarget(Creature speller, Creature target, int baseDamages,
    ...
    if (speller instanceof Player && ((Player) speller).getEquipment().getMainHandWeapon() != null
      ...
      Item fusionedItem = ItemService.newItem(((Player) speller).getEquipment().getMainHandWeapon().getFusionedItem(),
          1L);
      if (fusionedItem != null) {
    ...
    int damages = Math
        .round(baseDamages * (sgs.getCurrentStat(StatEnum.KNOWLEDGE) / 100.0F + totalBoostMagicalSkill / 1000.0F));
  ...
  public static int calculatePhysicalDodgeRate(Creature attacker, Creature attacked) {
    ...
    if (attacker instanceof Player && ((Player) attacker).getEquipment().getOffHandWeaponType() != null) {
      accuracy = Math.round(((attacker.getGameStats().getCurrentStat(StatEnum.MAIN_HAND_ACCURACY)
          + attacker.getGameStats().getCurrentStat(StatEnum.OFF_HAND_ACCURACY)) / 2));
    } else {
  ...
  public static double calculatePhysicalCriticalRate(Creature attacker, Creature attacked) {
    ...
    double criticalRate;
    if (attacker instanceof Player && ((Player) attacker).getEquipment().getOffHandWeaponType() != null) {
      critical = Math.round(((attacker.getGameStats().getCurrentStat(StatEnum.MAIN_HAND_CRITICAL)
          + attacker.getGameStats().getCurrentStat(StatEnum.OFF_HAND_CRITICAL)) / 2
  ...
  public static int calculateMagicalResistRate(Creature attacker, Creature attacked) {
    ...
    int resistRate = Math.round(((attacked.getGameStats().getCurrentStat(StatEnum.MAGICAL_RESIST)
        - attacker.getGameStats().getCurrentStat(StatEnum.MAGICAL_ACCURACY)) / 10));
  ...
  public static boolean calculateFallDamage(Player player, float distance) {
    ...
    if (distance >= FallDamageConfig.MINIMUM_DISTANCE_DAMAGE) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) new SM_ATTACK_STATUS((Creature) player, SM_ATTACK_STATUS.TYPE.DAMAGE, 0, damage));
    }
