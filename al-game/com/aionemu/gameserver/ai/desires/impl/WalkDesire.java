...
public class WalkDesire extends AbstractDesire implements MoveDesire {
  ...
  public WalkDesire(Npc npc, int power) {
    ...
    if (template != null) {
      ...
      PacketSendUtility.broadcastPacket((VisibleObject) this.owner,
          (AionServerPacket) new SM_EMOTION((Creature) this.owner, EmotionType.WALK));
  ...
  private void getNextTime() {
    ...
    if (this.route == null) {
      ...
      nextDelay = this.isRandomWalk ? Rnd.get(5, 60)
          : ((RouteStep) this.route.getRouteSteps().get(this.targetPosition)).getRestTime();
    }
