    ...
    public boolean handleDesire(AI<?> ai) {
      for (VisibleObject visibleObject : this.owner.getKnownList().getKnownObjects().values()) {
        ...
        if (visibleObject instanceof Creature) {
          ...
          if (!creature.getLifeStats().isAlreadyDead() && MathUtil.isIn3dRange((VisibleObject) this.owner,
              (VisibleObject) creature, this.owner.getAggroRange())) {
            ...
            Skill skill = SkillEngine.getInstance().getSkill((Creature) this.owner, skillId, 1,
                (VisibleObject) creature);
            skill.useSkill();
