  ...
  extends AionObject
{
  ...
  public void removePlayerFromGroup(Player player) {
    ...
    PacketSendUtility.broadcastPacket(player, (AionServerPacket)new SM_LEAVE_GROUP_MEMBER(), true, new ObjectFilter<Player>()
        {
      ...
      case LEAVE:
        ...
        for (Player member : getMembers()) {
          ...
          if (!subjective.equals(member))
            PacketSendUtility.sendPacket(member, (AionServerPacket)new SM_GROUP_MEMBER_INFO(this, subjective, groupEvent)); 
          if (size() > 1)
            PacketSendUtility.sendPacket(member, (AionServerPacket)SM_SYSTEM_MESSAGE.MEMBER_LEFT_GROUP(subjective.getName())); 
        } 
