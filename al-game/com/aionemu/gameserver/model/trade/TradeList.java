...
public class TradeList {
  ...
  public boolean calculateBuyListPrice(Player player, int modifier) {
    ...
    for (TradeItem tradeItem : this.tradeItems) {
      this.requiredKinah += player.getPrices().getKinahForBuy(tradeItem.getItemTemplate().getPrice())
          * tradeItem.getCount() * modifier / 100L;
    }
  ...
  public boolean calculateAbyssBuyListPrice(Player player) {
    ...
    for (TradeItem tradeItem : this.tradeItems) {
      ...
      if (alreadyAddedCount == null) {
        this.requiredItems.put(Integer.valueOf(itemId),
            Integer.valueOf(tradeItem.getItemTemplate().getAbyssItemCount()));
        continue;
      }
      this.requiredItems.put(Integer.valueOf(itemId),
          Integer.valueOf(alreadyAddedCount.intValue() + tradeItem.getItemTemplate().getAbyssItemCount()));
    }
