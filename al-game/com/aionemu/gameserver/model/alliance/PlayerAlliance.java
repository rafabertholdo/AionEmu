  ...
  extends AionObject
{
  ...
  public void removeMember(int memberObjectId) {
    ((PlayerAllianceGroup)this.allianceGroupForMember.get(Integer.valueOf(memberObjectId))).removeMember(memberObjectId);
  ...
  public boolean hasAuthority(int playerObjectId) {
    return (playerObjectId == this.captainObjectId || this.viceCaptainObjectIds.contains(Integer.valueOf(playerObjectId)));
  }
  ...
  public void swapPlayers(int playerObjectId1, int playerObjectId2) {
    PlayerAllianceGroup group1 = (PlayerAllianceGroup)this.allianceGroupForMember.get(Integer.valueOf(playerObjectId1));
    PlayerAllianceGroup group2 = (PlayerAllianceGroup)this.allianceGroupForMember.get(Integer.valueOf(playerObjectId2));
  ...
  public void setAllianceGroupFor(int memberObjectId, int allianceGroupId) {
    PlayerAllianceGroup leavingGroup = (PlayerAllianceGroup)this.allianceGroupForMember.get(Integer.valueOf(memberObjectId));
  ...
  public void onPlayerDisconnect(Player player) {
    PlayerAllianceMember allianceMember = (PlayerAllianceMember)this.allianceMembers.get(Integer.valueOf(player.getObjectId()));
    ...
    for (PlayerAllianceMember member : this.allianceMembers.values()) {
      ...
      if (member.isOnline()) {
        ...
        PacketSendUtility.sendPacket(member.getPlayer(), (AionServerPacket)SM_SYSTEM_MESSAGE.STR_FORCE_HE_BECOME_OFFLINE(player.getName()));
        PacketSendUtility.sendPacket(member.getPlayer(), (AionServerPacket)new SM_ALLIANCE_MEMBER_INFO(allianceMember, PlayerAllianceEvent.DISCONNECTED));
      } 
    } 
  }
  ...
  public Collection<PlayerAllianceMember> getMembersForGroup(int playerObjectId) {
    ...
    if (group == null) return (new FastMap()).values(); 
