    ...
    for (Friend friend : this.friends) {
      ...
      if (friend.isOnline()) {
        ...
        friendPlayer.getClientConnection()
            .sendPacket((AionServerPacket) new SM_FRIEND_UPDATE(this.player.getObjectId()));

        if (previousStatus == Status.OFFLINE) {

          friendPlayer.getClientConnection()
              .sendPacket((AionServerPacket) new SM_FRIEND_NOTIFY(0, this.player.getName()));
          continue;
        }
        if (status == Status.OFFLINE) {

          friendPlayer.getClientConnection()
              .sendPacket((AionServerPacket) new SM_FRIEND_NOTIFY(1, this.player.getName()));
        }
      }
    }
  }
