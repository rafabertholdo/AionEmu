    ...
    switch (skillId) {
      ...
      case 30002:
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330005));
        return;
      case 30003:
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330005));
      ...
      case 40009:
        PacketSendUtility.sendPacket(player,
            (AionServerPacket) new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330061));
        return;
    }
    PacketSendUtility.sendPacket(player,
        (AionServerPacket) new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1300050));
  }
        ...
        if (skillTemplate.getEffects() != null) {
          ...
          if (template = skillTemplate.getEffectTemplate(1) instanceof WeaponMasteryEffect) {
            ...
            if (skillLevels.get(wme.getWeaponType()) == null
                || ((Integer) skillLevels.get(wme.getWeaponType())).intValue() < wme.getBasicLvl()) {
        ...
        if (skillTemplate.getEffects() != null) {
          ...
          if (template = skillTemplate.getEffectTemplate(1) instanceof ArmorMasteryEffect) {
            ...
            if (skillLevels.get(ame.getArmorType()) == null
                || ((Integer) skillLevels.get(ame.getArmorType())).intValue() < ame.getBasicLvl()) {
