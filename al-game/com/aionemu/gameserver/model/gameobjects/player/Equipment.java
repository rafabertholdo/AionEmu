...
public class Equipment {
  ...
  public Item equipItem(int itemUniqueId, int slot) {
    ...
    if (itemTemplate.getLevel() > this.owner.getCommonData().getLevel()) {
      ...
      PacketSendUtility.sendPacket(this.owner,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_CANNOT_USE_ITEM_TOO_LOW_LEVEL_MUST_BE_THIS_LEVEL(
    ...
    synchronized (this.equipment) {
      ...
      for (ItemSlot possibleSlot : fastList) {
        ...
        if (this.equipment.get(Integer.valueOf(slotId)) == null
            || this.markedFreeSlots.contains(Integer.valueOf(slotId))) {
    ...
    synchronized (this.equipment) {
      ...
      if (this.equipment.get(Integer.valueOf(itemSlotToEquip)) != null) {
        ...
        log.error("CHECKPOINT : putting item to already equiped slot. Info slot: " + itemSlotToEquip + " new item: "
            ...
            + ((Item) this.equipment.get(Integer.valueOf(itemSlotToEquip))).getItemTemplate().getTemplateId());
  ...
  public Item unEquipItem(int itemUniqueId, int slot) {
    ...
    synchronized (this.equipment) {
      ...
      if (itemToUnequip.getItemTemplate().isArmor()
        ...
        PacketSendUtility.sendPacket(this.owner,
            (AionServerPacket) new SM_EMOTION(this.owner, EmotionType.POWERSHARD_OFF, 0, 0));
      }
  ...
  private boolean validateEquippedWeapon(Item item, boolean validateOnly) {
    ...
    switch (item.getItemTemplate().getWeaponType().getRequiredSlots()) {
      ...
      case 1:
        if (itemInMainHand != null && !this.owner.getSkillList().isSkillPresent(19)
            && !this.owner.getSkillList().isSkillPresent(360)) {
  ...
  public void usePowerShard(Item powerShardItem, int count) {
    ...
    if (powerShardItem.getItemCount() <= 0L) {
      ...
      List<Item> powerShardStacks = this.owner.getInventory()
  ...
  public void useArrow() {
    ...
    if (arrow == null
        || (!arrow.getItemTemplate().isArmor() && arrow.getItemTemplate().getArmorType() != ArmorType.ARROW)) {
      return;
    }
  ...
  public boolean isWeaponEquipped(WeaponType weaponType) {
    if (this.equipment.get(Integer.valueOf(ItemSlot.MAIN_HAND.getSlotIdMask())) != null
        && ((Item) this.equipment.get(Integer.valueOf(ItemSlot.MAIN_HAND.getSlotIdMask()))).getItemTemplate()
            .getWeaponType() == weaponType) {

      return true;
    }
    if (this.equipment.get(Integer.valueOf(ItemSlot.SUB_HAND.getSlotIdMask())) != null
        && ((Item) this.equipment.get(Integer.valueOf(ItemSlot.SUB_HAND.getSlotIdMask()))).getItemTemplate()
            .getWeaponType() == weaponType) {
  ...
  public boolean isArmorEquipped(ArmorType armorType) {
    int[] armorSlots = { ItemSlot.BOOTS.getSlotIdMask(), ItemSlot.GLOVES.getSlotIdMask(),
        ItemSlot.HELMET.getSlotIdMask(), ItemSlot.PANTS.getSlotIdMask(), ItemSlot.SHOULDER.getSlotIdMask(),
        ItemSlot.TORSO.getSlotIdMask() };

    for (int slot : armorSlots) {

      if (this.equipment.get(Integer.valueOf(slot)) != null
          && ((Item) this.equipment.get(Integer.valueOf(slot))).getItemTemplate().getArmorType() != armorType)
        return false;
    }
    return true;
  }
