...
public class Npc extends Creature {
  ...
  public Npc(int objId, NpcController controller, SpawnTemplate spawnTemplate, VisibleObjectTemplate objectTemplate) {
    super(objId, (CreatureController<? extends Creature>) controller, spawnTemplate, objectTemplate,
        new WorldPosition());
  ...
  public boolean isAtSpawnLocation() {
    return (MathUtil.getDistance(getSpawn().getX(), getSpawn().getY(), getSpawn().getZ(), getX(), getY(),
        getZ()) < 3.0D);
  }
  ...
  protected boolean isEnemyNpc(Npc visibleObject) {
    return (DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(getTribe(), visibleObject.getTribe())
        || DataManager.TRIBE_RELATIONS_DATA.isHostileRelation(getTribe(), visibleObject.getTribe()));
  }

  protected boolean isEnemyPlayer(Player visibleObject) {
    return (!DataManager.TRIBE_RELATIONS_DATA.isSupportRelation(getTribe(), visibleObject.getTribe())
        && !DataManager.TRIBE_RELATIONS_DATA.isFriendlyRelation(getTribe(), visibleObject.getTribe()));
  }

  protected boolean isEnemySummon(Summon visibleObject) {
    Player player = visibleObject.getMaster();
    if (player != null)
      return (!DataManager.TRIBE_RELATIONS_DATA.isSupportRelation(getTribe(), player.getTribe())
          && !DataManager.TRIBE_RELATIONS_DATA.isFriendlyRelation(getTribe(), player.getTribe()));
    return true;
  }
