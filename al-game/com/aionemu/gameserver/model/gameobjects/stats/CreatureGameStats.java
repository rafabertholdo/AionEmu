...
public class CreatureGameStats<T extends Creature>
{
  ...
  public void recomputeStats() {
    ...
    try {
      ...
      for (Map.Entry<StatEffectId, TreeSet<StatModifier>> modifiers : (Iterable<Map.Entry<StatEffectId, TreeSet<StatModifier>>>)this.statsModifiers.entrySet()) {
      ...
      for (Map.Entry<StatEnum, StatModifiers> entry : (Iterable<Map.Entry<StatEnum, StatModifiers>>)orderedModifiers.entrySet())
      {
        applyModifiers(entry.getKey(), entry.getValue());
      }
      
      setStat(StatEnum.ATTACK_SPEED, Math.round(getBaseStat(StatEnum.MAIN_HAND_ATTACK_SPEED) + getBaseStat(StatEnum.OFF_HAND_ATTACK_SPEED) * 0.25F), false);
