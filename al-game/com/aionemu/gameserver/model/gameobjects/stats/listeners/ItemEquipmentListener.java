...
public class ItemEquipmentListener
{
  ...
  private static void onItemSetPartEquipment(ItemSetTemplate itemSetTemplate, Player player) {
    ...
    for (PartBonus itempartbonus : itemSetTemplate.getPartbonus()) {
      ...
      if (itempartbonus.getCount() <= itemSetPartsEquipped && !player.getGameStats().effectAlreadyAdded((StatEffectId)setEffectId))
      {
  ...
  public static void onItemUnequipment(Item item, Player owner) {
    ...
    owner.getGameStats().endEffect((StatEffectId)ItemStatEffectId.getInstance(item.getItemTemplate().getTemplateId(), item.getEquipmentSlot()));
  ...
  private static void onItemSetPartUnequipment(ItemSetTemplate itemSetTemplate, Player player) {
    ...
    if (itemSetTemplate.getFullbonus() != null && previousItemSetPartsEquipped == itemSetTemplate.getFullbonus().getCount())
    {
      ...
      player.getGameStats().endEffect((StatEffectId)ItemSetStatEffectId.getInstance(itemSetTemplate.getId(), itemSetTemplate.getFullbonus().getCount() + 1));
    }
    ...
    for (PartBonus itempartbonus : itemSetTemplate.getPartbonus()) {
      ...
      {
        player.getGameStats().endEffect((StatEffectId)ItemSetStatEffectId.getInstance(itemSetTemplate.getId(), itempartbonus.getCount()));
      }
    } 
  }
