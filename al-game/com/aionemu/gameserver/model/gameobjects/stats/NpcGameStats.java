...
public class NpcGameStats extends CreatureGameStats<Npc> {
  ...
  public NpcGameStats(Npc owner) {
    ...
    initStat(StatEnum.MAXHP,
        nst.getMaxHp() + Math.round(owner.getObjectTemplate().getHpGauge() * 1.5F * owner.getLevel()));

    initStat(StatEnum.MAXMP, nst.getMaxMp());

    initStat(StatEnum.ATTACK_SPEED, 2000);
    initStat(StatEnum.PHYSICAL_DEFENSE,
        Math.round((nst.getPdef() / owner.getLevel() - 1.0F) * nst.getPdef() + (10 * owner.getLevel())));
  ...
  public void recomputeStats() {
    ...
    if (newRunSpeed != this.currentRunSpeed) {
      ...
      PacketSendUtility.broadcastPacket((VisibleObject) this.owner,
          (AionServerPacket) new SM_EMOTION((Creature) this.owner, EmotionType.START_EMOTE2, 0, 0));
    }
