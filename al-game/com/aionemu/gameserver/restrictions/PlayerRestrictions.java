...
public class PlayerRestrictions extends AbstractRestrictions {
  public boolean canAffectBySkill(Player player, VisibleObject target) {
    ...
    if (player.getEffectController().isAbnormalState(EffectId.CANT_ATTACK_STATE)
        && skill.getSkillTemplate().getSkillId() != 1968) {
      return false;
    }
    if (player.isInState(CreatureState.PRIVATE_SHOP)) {

      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_MSG_CANNOT_USE_ITEM_WHILE_PRIVATE_STORE);
      return false;
    }
  ...
  public boolean canInviteToGroup(Player player, Player target) {
    ...
    if (target.isInGroup()) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.PLAYER_IN_ANOTHER_GROUP(target.getName()));
      return false;
    }
    if (target.isInAlliance()) {

      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_FORCE_ALREADY_OTHER_FORCE(target.getName()));
      return false;
    }
  ...
  public boolean canInviteToAlliance(Player player, Player target) {
    ...
    if (alliance != null && alliance.isFull()) {
      ...
      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_PARTY_ALLIANCE_CANT_ADD_NEW_MEMBER());
      return false;
    }
    if (alliance != null && !alliance.hasAuthority(player.getObjectId())) {

      PacketSendUtility.sendPacket(player,
          (AionServerPacket) SM_SYSTEM_MESSAGE.STR_PARTY_ALLIANCE_ONLY_PARTY_LEADER_CAN_LEAVE_ALLIANCE());
      return false;
    }
    if (target.sameObjectId(player.getObjectId())) {
    ...
    if (target.isInGroup()) {
      ...
      if (target != targetGroup.getGroupLeader()) {
        ...
        PacketSendUtility.sendPacket(player, (AionServerPacket) SM_SYSTEM_MESSAGE
            .STR_FORCE_INVITE_PARTY_HIM(target.getName(), targetGroup.getGroupLeader().getName()));
        return false;
      }
      if (alliance != null && targetGroup.size() + alliance.size() >= 24) {

        PacketSendUtility.sendPacket(player,
            (AionServerPacket) SM_SYSTEM_MESSAGE.STR_FORCE_INVITE_FAILED_NOT_ENOUGH_SLOT());
        return false;
      }
    }
