...
public class AuraEffect extends EffectTemplate {
  ...
  public void onPeriodicAction(Effect effect) {
    ...
    if (effector.isInAlliance()) {
      ...
      for (PlayerAllianceMember allianceMember : effector.getPlayerAlliance()
        ...
        if (allianceMember.isOnline() && MathUtil.isIn3dRange((VisibleObject) effector,
            (VisibleObject) allianceMember.getPlayer(), (this.distance + 4))) {
  ...
  private void applyAuraTo(Player effector) {
    ...
    Effect e = new Effect((Creature) effector, (Creature) effector, template, template.getLvl(),
        template.getEffectsDuration());
    e.initialize();
    e.applyEffect();
    PacketSendUtility.broadcastPacket((VisibleObject) effector,
        (AionServerPacket) new SM_MANTRA_EFFECT(effector, this.skillId));
  }
