...
public class TaskManagerFromDB {
  ...
  public TaskManagerFromDB() {
    ...
    log.info(
        "Loaded " + this.tasksList.size() + " task" + ((this.tasksList.size() > 1) ? "s" : "") + " from the database");
    addTasks();
  ...
  private void addTasks() {
    ...
    this.tasksList.add(new TaskFromDB(maxId + 1, "update_abyss_rank", "FIXED_IN_TIME",
        new Timestamp(System.currentTimeMillis()), "00:00:00", 0, null));
  }
  ...
  private void registerTasks() {
    for (TaskFromDB task : this.tasksList) {
      ...
      if (this.handlers.get(task.getName()) != null) {
        ...
        Class<? extends TaskFromDBHandler> tmpClass = (Class) ((TaskFromDBHandler) this.handlers.get(task.getName()))
            .getClass();
