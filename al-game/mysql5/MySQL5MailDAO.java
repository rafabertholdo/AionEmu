...
public class MySQL5MailDAO extends MailDAO {
    ...
    public Mailbox loadPlayerMailbox(Player player) {
        ...
        try {
            ...
            PreparedStatement stmt = con.prepareStatement("SELECT * FROM mail WHERE mailRecipientId = ?");
                    ...
                    for (Item item : mailboxItems) {
                        ...
                        if (item.getObjectId() == attachedItemId) {
                            ...
                            if (item.getItemTemplate().isArmor() || item.getItemTemplate().isWeapon()) {
                                ((ItemStoneListDAO) DAOManager.getDAO(ItemStoneListDAO.class))
                                        .load(Collections.singletonList(item));
                            }
                            attachedItem = item;
                        }
                    }
    ...
    private List<Item> loadMailboxItems(int playerId) {
        List<Item> mailboxItems = new ArrayList<Item>();
        ...
        try {
            ...
            PreparedStatement stmt = con
                    .prepareStatement("SELECT * FROM inventory WHERE `itemOwner` = ? AND `itemLocation` = 127");
            ...
            while (rset.next()) {
                ...
                Item item = new Item(playerId, itemUniqueId, itemId, itemCount, itemColor, (isEquiped == 1),
                        (isSoulBound == 1), slot, StorageType.MAILBOX.getId(), enchant, itemSkin, fusionedItem);
    ...
    private boolean saveLetter(Timestamp time, Letter letter) {
        ...
        try {
            ...
            PreparedStatement stmt = con.prepareStatement(
                    "INSERT INTO `mail` (`mailUniqueId`, `mailRecipientId`, `senderName`, `mailTitle`, `mailMessage`, `unread`, `attachedItemId`, `attachedKinahCount`, `express`, `recievedTime`) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    private boolean updateLetter(Timestamp time, Letter letter) {
        ...
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement(
                    "UPDATE mail SET  unread=?, attachedItemId=?, attachedKinahCount=?, recievedTime=? WHERE mailUniqueId=?");
    ...
    public int[] getUsedIDs() {
        ...
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("SELECT mailUniqueId FROM mail", 1004, 1007);
            ResultSet rset = stmt.executeQuery();
            rset.last();
 ...
 * !\mysql5\MySQL5MailDAO.class Java compiler version: 6 (50.0) JD-Core Version:
 * 1.1.3
 */